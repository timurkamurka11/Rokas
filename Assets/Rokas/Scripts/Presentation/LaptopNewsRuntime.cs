...
namespace Rokas.Presentation
{
    ...
    public sealed class LaptopNewsRuntime : MonoBehaviour
    {
        ...
        private static byte[] LoadEmbeddedBytes(string resourcePrefix, int chunkCount)
        {
            ...
            {
                var chunk = Resources.Load<TextAsset>(resourcePrefix + "_" + i.ToString("00"));
                if (!chunk) return null;
                base64.Append(chunk.text.Trim());
            }
            try { return Convert.FromBase64String(base64.ToString()); }
        ...
        {
            var card = Surface(parent, name, x, y, w, h, 12, new Color(.055f, .09f, .112f, .94f), true).rectTransform;
            ui.Box(card, "Accent", 0, 0, 4, h, new Color(Jade.r, Jade.g, Jade.b, .65f));
            ui.Label(card, "Meta", meta, 18, 8, w - 36, 18, 11, Gold);
            ui.Label(card, "Title", title, 18, 27, w - 36, h - 31, 16, White, false, TextAnchor.UpperLeft);
        }
